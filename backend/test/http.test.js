...
test('backend health is lazy and browser actions are delegated',async()=>{
  const calls=[];let stops=0;const browser={status:()=>({state:'idle',operations:['navigate']}),connect:async()=>({state:'ready'}),execute:async(operation,args)=>{calls.push({operation,args});return{ok:true}},stop:async()=>{stops++}};
  ...
  try{
    ...
    const stopped=await fetch(`${base}/api/browser/session`,{method:'DELETE',headers:{origin:'http://127.0.0.1:5173'}}).then(response=>response.json());assert.equal(stopped.stopped,true);assert.equal(stops,1);
