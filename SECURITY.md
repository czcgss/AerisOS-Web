...
Future runs an emulated Linux system and can connect an AI model to operating-system tools. Treat it as experimental software, not as a hardened security boundary.
...
- High-risk Agent tools require a Future approval dialog, but contributors must still validate parameters and keep privileged operations narrowly scoped.
...
- Clearing browser site data removes Future machine snapshots and local application state.

Please avoid using real secrets or irreplaceable data while evaluating Future.
