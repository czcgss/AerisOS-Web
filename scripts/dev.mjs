...
const backendPort=Number.parseInt(process.env.FUTURE_BACKEND_PORT,10)||4318,backendUrl=`http://127.0.0.1:${backendPort}`;
const existingBackend=async()=>{
  ...
  if(response.ok&&payload.service==='future-backend'&&payload.protocolVersion===BACKEND_PROTOCOL_VERSION)return true;
  if(response.ok&&payload.service==='future-backend')throw new Error(`An older Future Backend is already running on port ${backendPort}. Stop the previous pnpm dev process and try again.`);
  throw new Error(`Port ${backendPort} is already in use by a service that is not Future Backend.`);
};

try{
  if(await existingBackend())console.log(`Reusing Future backend at ${backendUrl}`);
  else start('Future backend',['backend/src/server.js']);
