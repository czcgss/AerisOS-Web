...
const fail=message=>{throw new Error(`Invalid Future widget package: ${message}`)};
...
export function validateWidgetPackage(source){
  ...
  for(const path of ['locales/en.json','locales/zh.json']){if(typeof files[path]!=='string')fail(`missing locale file “${path}”`);try{if(!plain(JSON.parse(files[path])))fail(`locale file “${path}” must contain an object`)}catch(error){if(error.message.startsWith('Invalid Future'))throw error;fail(`locale file “${path}” is not valid JSON`)}}
  ...
  if(!/\bFutureWidget\s*\.\s*ready\b/.test(files[entry.script]))fail('entry script must wait for FutureWidget.ready');
  ...
  for(const [source,permission] of Object.entries({calendar:'calendar.read',reminders:'reminders.read',weather:'weather.read',metrics:'metrics.read',music:'music.read'})){
    if(new RegExp(`\\bFutureWidget\\s*\\.\\s*data\\s*\\.\\s*(?:get|subscribe)\\s*\\(\\s*['"]${source}['"]`).test(script))requires(permission);
  }
  if(/\bFutureWidget\s*\.\s*music\s*\./.test(script))requires('music.control');
  if(/\bFutureWidget\s*\.\s*state\s*\./.test(script))requires('storage');
  if(/\bFutureWidget\s*\.\s*apps\s*\.\s*open\s*\(/.test(script))requires('app.open');
