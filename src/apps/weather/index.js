...
export default{
  ...
  mount(root,{weather,i18n,kernel,agentContext}){
    ...
    const draw=()=>{
      ...
      const locationId=`${s.location.latitude},${s.location.longitude}`;agentContext.set({appId:'weather',label:i18n.t('weather'),resource:{kind:'weather-location',entityType:'weather.location',id:locationId,uri:`future://weather/locations/${encodeURIComponent(locationId)}`,name:s.location.name,metadata:{...s.location,temperature:current?.temperature_2m,condition:condition.label}}});bind();
    };
