...
export default {
  ...
  mount(root, { aiAgent, i18n, kernel, dialog, shell, clipboard, tools, notifications, agentContext, agentEntry, agentTasks }) {
    ...
    const contextMarkup=()=>{const context=agentContext.snapshot(),windows=shell.windowManager.contextWindows();if(!context&&!windows.length)return'';const app=tools.registry.get(context?.appId),resource=context?.resource,label=resource?.name||resource?.path||resource?.date||context?.label||i18n.t('chooseContext'),desktop=resource?.kind==='desktop';return`<div class="ai-native-context-wrap"><div class="ai-native-context"><button class="ai-context-current" data-ai-context-selector aria-expanded="${contextMenuOpen}"><span class="app-icon app-icon-${app?.color||'blue'}">${icon(app?.icon||(desktop?'desktop':'sparkles'),14)}</span><span><small>${i18n.t('systemContext')}</small><strong>${esc(label)}</strong></span>${context?.selection?.text?`<em>${i18n.t('selectedText')}</em>`:''}${icon('chevron',10)}</button>${context?`<button data-ai-clear-context aria-label="${i18n.t('clearContext')}">${icon('close',10)}</button>`:''}</div>${contextMenuOpen?`<section class="ai-context-picker"><header><strong>${i18n.t('chooseContext')}</strong><small>${i18n.t('chooseContextCopy')}</small></header><button data-ai-context-desktop class="${desktop?'selected':''}"><span class="app-icon app-icon-blue">${icon('desktop',14)}</span><span><strong>${i18n.t('desktop')}</strong><small>${i18n.t('desktopContext')}</small></span>${desktop?icon('check',11):''}</button>${windows.map(item=>`<button data-ai-context-window="${esc(item.id)}" class="${context?.windowId===item.id?'selected':''}"><span class="app-icon app-icon-${item.color}">${icon(item.icon,14)}</span><span><strong>${esc(item.title)}</strong><small>${esc(item.path||i18n.t(item.minimized?'minimizedWindow':'openWindow'))}</small></span>${context?.windowId===item.id?icon('check',11):''}</button>`).join('')}</section>`:''}</div>`};
    const taskMarkup=()=>{const task=activeId?agentTasks.forSession(activeId)[0]:null;if(!task||task.status==='completed'&&!task.steps.length)return'';const statusKey=task.status==='running'?'agentTaskRunning':task.status==='failed'?'agentTaskFailed':task.status==='cancelled'?'agentTaskCancelled':'agentTaskCompleted';return`<section class="ai-native-task ${task.status}"><header><span>${icon('aerisAi',14)}</span><div><small>${i18n.t('systemTask')}</small><strong>${esc(task.title)}</strong></div><em>${i18n.t(statusKey)}</em></header>${task.steps.length?`<div>${task.steps.map(step=>{const app=tools.registry.get(step.appId),warning=['failed','denied','cancelled'].includes(step.phase);return`<span class="phase-${esc(step.phase)}"><i class="app-icon app-icon-${app?.color||'grey'}">${icon(app?.icon||'wrench',11)}</i><b>${esc(step.label||step.operation)}</b>${icon(step.phase==='completed'?'check':warning?'warning':'chevron',10)}</span>`}).join('')}</div>`:''}</section>`};
    ...
    render();
    return () => { if(streamingFrame)cancelAnimationFrame(streamingFrame);offReady(); offChanged(); offAgent(); offCapability(); offNotifications(); offContext(); offTasks(); offEntry(); offLocale(); };
  },
};
