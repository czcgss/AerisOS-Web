...
import { contextViewMarkup, workspaceMarkup, workspacePickerMarkup } from './AgentWorkspace.js';
...
import {automationSettingsMarkup,taskWorkspaceMarkup,taskWorkspaceSignature} from './SystemTasks.js';
...
const WORKSPACE_PREFS_KEY = 'future.ai.workspace';
...
const readWorkspacePrefs = () => {
  try {
    ...
    return {
      ...
      view: ['workflow','tasks','activity','context'].includes(stored.view) ? stored.view : 'activity',
    };
  } catch {
...
export default {
  id: 'ai', title: 'aiAssistant', icon: 'futureAi', color: 'ai', width: 1280, height: 760,
  singleInstance: true, dockLeading: true,
  mount(root, { aiAgent, agentRegistry, multiAgent, i18n, kernel, dialog, shell, clipboard, tools, notifications, agentContext, agentModes, agentEntry, queryUser, userdata, system, settings, weather, music, browser, browserAutomation, metrics, machine, skillRegistry, systemTasks, automations, operationHistory }) {
    ...
    let taskWorkspaceView='active',automationFormOpen=false,automationTriggerType='daily';
    ...
    const modeLabel=id=>{const mode=agentModes?.get(id),key=mode?.nameKey;return key?i18n.t(key):mode?.name||id};
    const sessionModeLocked=session=>Boolean(session?.turns?.length||session?.messages?.length||session?.busy);
    const modeSelectorMarkup=session=>{const locked=sessionModeLocked(session),active=session?.agentMode||'general',mode=agentModes?.get(active);if(mode?.selectable===false)return`<span class="ai-composer-mode ai-composer-mode-static">${icon('focus',12)}${esc(modeLabel(active))}</span>`;return`<label class="ai-composer-mode" title="${i18n.t(locked?'agentModeLocked':'agentMode')}">${icon('focus',12)}<select data-ai-composer-mode ${locked?'disabled':''}>${aiAgent.modeOptions().map(item=>`<option value="${esc(item.id)}" ${item.id===active?'selected':''}>${esc(modeLabel(item.id))}</option>`).join('')}</select>${icon('chevron',9)}</label>`};
    ...
    const semanticContextWindows=()=>shell.windowManager.contextWindows().map(item=>{const saved=agentContext.forWindow(item.id),resource=saved?.resource,resourceLabel=resource?.name||resource?.path||resource?.date||'';return{...item,resource,resourceLabel,contextLabel:resourceLabel&&resourceLabel!==item.title?`${item.title} · ${resourceLabel}`:item.title}});
    const focusActivityContext=app=>{if(!app)return;agentContext.set({appId:app.id,label:i18n.t(app.title),resource:{kind:'application-view',id:`future://agent/apps/${app.id}`,uri:`future://agent/apps/${app.id}`,name:i18n.t(app.title),metadata:{appId:app.id,surface:'agent-activity',compact:true}}})};
    ...
    const friendlyError = error => /guest command|mkdir -p|__future_/i.test(error?.message || '') ? i18n.t('conversationSaveFailed') : (error?.message || String(error));
    ...
    const notificationMarkup=()=>{const {items}=notifications.snapshot();return`<div class="ai-notification-backdrop" data-ai-close-notifications></div><section class="ai-notification-menu"><header><div><strong>${i18n.t('systemNotifications')}</strong><small>${i18n.t('notificationCenterCopy')}</small></div>${items.length?`<button data-ai-clear-notifications>${i18n.t('clearAll')}</button>`:''}</header><div>${items.length?items.map(item=>{const app=tools.registry.get(item.appId),approval=item.type==='agent'&&item.category==='approval'&&item.agent?.status==='approval'&&tools.execution(item.agent.toolCallId)?.phase==='approval',actions=approval?`<footer class="ai-notification-actions"><button data-ai-notification-deny="${esc(item.agent.toolCallId)}">${i18n.t('deny')}</button><button class="primary" data-ai-notification-approve="${esc(item.agent.toolCallId)}">${i18n.t('approve')}</button></footer>`:item.type==='agent'?`<button class="ai-notification-agent" data-ai-open-agent-notification="${item.id}">${i18n.t('viewConversation')}</button>`:item.context?`<button class="ai-notification-agent" data-ai-handle-notification="${item.id}">${icon('futureAi',11)} ${i18n.t('prepareWithFuture')}</button>`:'';return`<article class="${item.read?'':'unread'}" data-notification-updated="${item.updatedAt||item.createdAt}"><button data-ai-open-notification="${item.id}"><span class="app-icon app-icon-${app?.color||'blue'}">${icon(app?.icon||'bell',18)}</span><div><small>${esc(item.type==='agent'?(item.agent?.sessionTitle||i18n.t('futureAI')):(app?i18n.t(app.title):i18n.t('system')))}</small><strong>${esc(item.title)}</strong><p>${esc(item.message)}</p><time>${new Intl.DateTimeFormat(i18n.t('dateFormat'),{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'}).format(new Date(item.createdAt))}</time></div></button>${actions}<button data-ai-dismiss-notification="${item.id}" aria-label="${i18n.t('dismiss')}">${icon('close',11)}</button></article>`}).join(''):`<div class="ai-notification-empty">${icon('bell',27)}<strong>${i18n.t('noNotifications')}</strong><small>${i18n.t('notificationEmptyCopy')}</small></div>`}</div></section>`};
    ...
    const bindNotificationControls=(scope=root)=>{
      ...
      const openAgentNotification=item=>{if(!item?.agent?.sessionId)return;closeNotificationPanel();notifications.markRead(item.id);agentEntry.open({sessionId:item.agent.sessionId,source:'agent-notification',target:{turnId:item.agent.turnId,toolCallId:item.agent.toolCallId}})};
      scope.querySelectorAll('[data-ai-open-notification]').forEach(button=>button.onclick=()=>{const item=notifications.snapshot().items.find(entry=>entry.id===button.dataset.aiOpenNotification);if(item?.type==='agent')return openAgentNotification(item);closeNotificationPanel();if(!item)return;notifications.markRead(item.id);const app=activateApp(item.appId,'',{notificationId:item.id,context:item.context});if(app)focusActivityContext(app);render({preserveComposer:true,preserveConversation:true,focusComposer:false})});
      scope.querySelectorAll('[data-ai-open-agent-notification]').forEach(button=>button.onclick=event=>{event.stopPropagation();openAgentNotification(notifications.snapshot().items.find(entry=>entry.id===button.dataset.aiOpenAgentNotification))});
      scope.querySelectorAll('[data-ai-notification-approve],[data-ai-notification-deny]').forEach(button=>button.onclick=event=>{event.stopPropagation();tools.resolveApproval(button.dataset.aiNotificationApprove||button.dataset.aiNotificationDeny,Boolean(button.dataset.aiNotificationApprove))});
      scope.querySelectorAll('[data-ai-handle-notification]').forEach(button=>button.onclick=event=>{event.stopPropagation();const item=notifications.snapshot().items.find(entry=>entry.id===button.dataset.aiHandleNotification);if(!item)return;closeNotificationPanel();notifications.markRead(item.id);agentEntry.open({prompt:i18n.t('prepareWithFuturePrompt').replace('{title}',item.title),context:item.context,source:'notification'})});
    };
    const refreshNotificationPanel=state=>{if(!notificationOpen)return syncNotificationIndicator(state);const menu=root.querySelector('.ai-notification-menu'),domItems=[...(menu?.querySelectorAll('[data-ai-open-notification]')||[])].map(node=>{const article=node.closest('article');return`${node.dataset.aiOpenNotification}:${article?.dataset.notificationUpdated||''}`}),nextItems=state.items.map(item=>`${item.id}:${item.updatedAt||item.createdAt}`);if(menu&&domItems.length===nextItems.length&&domItems.every((value,index)=>value===nextItems[index])){for(const item of state.items)menu.querySelector(`[data-ai-open-notification="${CSS.escape(item.id)}"]`)?.closest('article')?.classList.toggle('unread',!item.read);syncNotificationIndicator(state);return}root.querySelectorAll('.ai-notification-menu,.ai-notification-backdrop').forEach(node=>node.remove());const workspace=root.querySelector('.ai-workspace');if(workspace){workspace.insertAdjacentHTML('beforeend',notificationMarkup());bindNotificationControls(workspace)}syncNotificationIndicator(state)};
    ...
    const contextMarkup=()=>{
      const context=agentContext.snapshot(),windows=semanticContextWindows(),availableWindows=[...windows,...activityContextWindows()];
    ...
    const turnMarkup = (turn, index, lastTurnIndex, session) => {
      ...
      const userActions=actionsAvailable?`<footer class="ai-message-actions"><button data-ai-copy-turn="${esc(turn.id)}" data-ai-copy-role="user" title="${i18n.t('copyMessage')}">${icon('copy',14)}</button>${turn.source!=='automation'&&index===lastTurnIndex&&!session.busy?`<button data-ai-edit-turn="${esc(turn.id)}" title="${i18n.t('editMessage')}">${icon('textedit',14)}</button>`:''}</footer>`:'';
      ...
      const automationLabel=turn.source==='automation'?`<header class="ai-automation-instruction">${icon('timer',11)} ${i18n.t('automationInstruction')}</header>`:'';
      const userBody=editing?`<div class="ai-inline-editor"><textarea data-ai-inline-edit rows="2">${esc(editDraft)}</textarea><footer><button data-ai-cancel-inline-edit>${i18n.t('cancel')}</button><button class="ai-primary" data-ai-submit-inline-edit>${i18n.t('send')}</button></footer></div>`:`${automationLabel}${attachmentListMarkup(turn.user?.attachments)}${userText?`<div class="ai-message-content" data-copyable>${renderText(userText)}</div>`:''}${userActions}`;
      return `<section class="ai-turn ${actionsAvailable?'':'ai-turn-running'}" data-ai-turn="${esc(turn.id)}">
        <article class="ai-message ai-message-user ${turn.source==='automation'?'ai-message-automation':''}"><span class="ai-message-avatar">${icon(turn.source==='automation'?'timer':'user',16)}</span><div class="ai-message-body ${editing?'editing':''}">${userBody}</div></article>
        ${showAnswer?`<article class="ai-message ai-message-assistant" data-ai-turn-answer="${esc(turn.id)}"><span class="ai-message-avatar">${icon('futureAi',17)}</span><div class="ai-message-body"><header>${i18n.t('futureAI')}</header><div class="ai-turn-response">${answer}</div>${assistantActions}</div></article>`:''}
      </section>`;
    };
    const conversationMarkup=(state,session,configured)=>{
      const turns=session?.turns||[];displayedTurns=turns;
      if(!state.ready)return`<div class="ai-center-state"><span class="ai-orb waiting">${icon('futureAi',30)}</span><h2>${i18n.t('preparingAI')}</h2>${state.error?errorMarkup(state.error,'ai-center-error'):`<p>${i18n.t('waitingForLinuxAI')}</p>`}</div>`;
      if(!configured)return`<div class="ai-center-state"><span class="ai-orb">${icon('futureAi',30)}</span><h2>${i18n.t('meetFutureAI')}</h2><p>${i18n.t('configureAICopy')}</p><button data-ai-settings class="ai-primary">${i18n.t('configureAI')}</button></div>`;
    ...
    const sessionIndicator=(item,notificationState=notifications.snapshot())=>{const approval=tools.pendingApproval(item.id),input=queryUser.pendingForSession(item.id),unread=notificationState.items.some(note=>note.type==='agent'&&!note.read&&note.agent?.sessionId===item.id);if(approval)return{state:'approval',label:i18n.t('agentWaitingApproval')};if(input)return{state:'input',label:i18n.t('agentInputRequired')};if(item.busy||item.streaming)return{state:'running',label:i18n.t('agentStatus_running')};if(unread)return{state:'unread',label:i18n.t('agentFinished')};return null};
    const syncSessionIndicators=()=>{const state=aiAgent.snapshot(),notificationState=notifications.snapshot();for(const button of root.querySelectorAll('[data-ai-session]')){const item=state.sessions.find(entry=>entry.id===button.dataset.aiSession),indicator=item?sessionIndicator(item,notificationState):null,dot=button.querySelector('[data-ai-session-indicator]'),status=button.querySelector('[data-ai-session-status]'),title=button.querySelector('strong'),date=button.querySelector('small>span');if(title?.firstChild&&item)title.firstChild.nodeValue=item.title;if(date&&item)date.textContent=new Intl.DateTimeFormat(i18n.t('dateFormat'),{month:'short',day:'numeric'}).format(item.updatedAt);if(dot){dot.hidden=!indicator;dot.dataset.state=indicator?.state||''}if(status){status.hidden=!indicator;status.textContent=indicator?.label||''}}};
    const focusNotificationTarget=target=>{if(!target)return;requestAnimationFrame(()=>{const toolId=String(target.toolCallId||''),turnId=String(target.turnId||''),node=[...root.querySelectorAll('[data-tool-call]')].find(item=>item.dataset.toolCall===toolId)||[...root.querySelectorAll('[data-ai-turn]')].find(item=>item.dataset.aiTurn===turnId);if(!node)return;node.scrollIntoView({block:'center',behavior:document.documentElement.dataset.reduceMotion==='true'?'auto':'smooth'});node.classList.remove('agent-notification-target');requestAnimationFrame(()=>node.classList.add('agent-notification-target'));clearTimeout(workspaceHighlightTimer);workspaceHighlightTimer=setTimeout(()=>node.classList.remove('agent-notification-target'),1800)})};
    ...
    const render = ({ preserveComposer = false, preserveConversation = false, focusSearch = false, focusComposer = preserveComposer } = {}) => {
      ...
      const state = aiAgent.snapshot(), session = current(), configured = Boolean(aiAgent.config().apiKey), notificationState=notifications.snapshot(),taskCenterState=systemTasks.snapshot();
      kernel.bus.emit('ai:surface-session',{surface:'full',sessionId:activeId||''});
      ...
      const workspaceContext=agentContext.snapshot(),contextWindows=semanticContextWindows(),sessionWorkflows=multiAgent.listWorkflows(activeId).filter(item=>item.nodes?.length>1),expandedWorkflowIds=new Set(sessionWorkflows.filter((workflow,index)=>workflowFoldState.has(workflow.id)?workflowFoldState.get(workflow.id):workflow.status==='running'||index===0).map(workflow=>workflow.id));
      ...
      const localeSignature=i18n.t('dateFormat'),contextSignature=workspaceView==='context'?JSON.stringify({context:workspaceContext,windows:contextWindows.map(item=>({id:item.id,title:item.title,path:item.path,minimized:item.minimized}))}):workspaceView==='activity'?JSON.stringify({activityAppId,activityAppIds,target:activityTarget?.id||activityTarget?.result?.id||activityTarget?.path||''}):'';
      const workflowSignature=sessionWorkflows.map(workflow=>`${workflow.id}:${workflow.updatedAt}:${workflow.progress}:${expandedWorkflowIds.has(workflow.id)?1:0}:${workflow.nodes.map(node=>`${node.id}:${node.phase}:${node.progress}:${(node.tools||[]).map(tool=>`${tool.id}:${tool.phase}:${tool.updatedAt||tool.finishedAt||0}`).join(',')}`).join(';')}`).join('|')||'empty',tasksSignature=taskWorkspaceSignature(systemTasks,operationHistory,{automations,view:taskWorkspaceView}),signature=workspaceView==='workflow'?`workflow:${workflowSignature}`:workspaceView==='tasks'?`tasks:${localeSignature}:${tasksSignature}`:workspaceView==='context'?`context:${localeSignature}:${contextSignature}`:`activity:${localeSignature}:${contextSignature}`,reuseWorkspace=Boolean(previousWorkspace&&workspaceVisible&&previousWorkspace.dataset.workspaceSignature===signature),animateWorkspace=!previousWorkspace;
      const taskMarkup=workspaceView==='tasks'?taskWorkspaceMarkup({systemTasks,operationHistory,automations,registry:tools.registry,i18n,view:taskWorkspaceView}):'';
      const shellMarkup=`<div class="system-app ai-system-app ${workspaceVisible?'has-app-workspace':''}" style="--agent-workspace-width:${workspaceWidth}px">
        <aside class="ai-sidebar">
          <header><span class="ai-brand-icon">${icon('futureAi', 21)}</span><strong>${i18n.t('futureAI')}</strong></header>
          ...
          <nav class="ai-session-list">${sessions.length ? sessions.map(item => {const indicator=sessionIndicator(item,notificationState),automated=item.origin==='automation';return`<div class="ai-session-row ${item.id === activeId ? 'selected' : ''} ${automated?'automation-session':''}"><button data-ai-session="${item.id}"><span>${icon(automated?'timer':'message', 15)}</span><span><strong>${esc(item.title)}${automated?`<em class="ai-session-origin">${i18n.t('automationConversation')}</em>`:''}</strong><small><span>${new Intl.DateTimeFormat(i18n.t('dateFormat'), { month: 'short', day: 'numeric' }).format(item.updatedAt)}</span><em class="ai-session-mode">${esc(modeLabel(item.agentMode||'general'))}</em><em data-ai-session-status ${indicator?'':'hidden'}>${esc(indicator?.label||'')}</em></small></span><i data-ai-session-indicator data-state="${indicator?.state||''}" ${indicator?'':'hidden'}></i></button><button data-ai-delete="${item.id}" title="${i18n.t('deleteChat')}">${icon('delete', 13)}</button></div>`}).join('') : `<div class="ai-sidebar-empty">${i18n.t(query ? 'noSearchResults' : 'noConversations')}</div>`}</nav>
        ...
        <section class="ai-workspace">
          <header class="ai-toolbar"><div><strong>${session ? esc(session.title) : i18n.t('futureAI')}</strong><small>${session?.origin==='automation'?`${i18n.t('automationConversation')} · ${esc(session.automation?.name||session.title)}`:(configured ? esc(aiAgent.config().model) : i18n.t('notConnected'))}</small></div>${session?.origin==='automation'?`<span class="ai-automation-badge">${icon('timer',12)} ${i18n.t('automationConversation')}</span>`:''}<span class="ai-local-badge">${icon('lock', 12)} ${i18n.t('storedOnThisComputer')}</span><button data-ai-switch-compact title="${i18n.t('compactAgentView')}">${icon('compactView',17)}</button><button class="ai-applications-button ${activityAppsOpen?'selected':''}" data-ai-applications aria-pressed="${activityAppsOpen}" title="${i18n.t('openApplication')}">${icon('grid', 17)}</button><button class="ai-notification-button ${notificationOpen?'selected':''} ${notificationState.unread?'has-unread':''}" data-ai-notifications title="${i18n.t('notifications')}" aria-label="${notificationState.unread?`${i18n.t('notifications')}, ${notificationState.unread}`:i18n.t('notifications')}">${icon('bell', 17)}<i class="${notificationState.unread?'visible':''}" data-ai-notification-dot></i></button>${workspacePickerMarkup(i18n,{view:workspaceView,menuOpen:workspaceViewMenuOpen,workspaceOpen:workspaceVisible,runningTasks:taskCenterState.running})}</header>
        ...
        ${workspaceVisible?workspaceMarkup(tools,i18n,{animate:animateWorkspace,signature,view:workspaceView,context:workspaceContext,windows:contextWindows,activityApps,activityAppId,workflows:sessionWorkflows,expandedWorkflowIds,taskMarkup}):''}
      </div>`;
    ...
    const settingsMarkup = () => {
      ...
      const skillContent=`<div class="ai-tool-settings ai-skill-settings"><header><div><h3>${i18n.t('skillSettings')}</h3><p>${i18n.t('skillAccessCopy')}</p></div><button data-ai-import-skill>${icon('folder',12)} ${i18n.t('importSkillFolder')}</button><input data-ai-skill-folder type="file" webkitdirectory directory multiple hidden></header><div>${skillRegistry.list().map(skill=>`<button class="ai-tool-permission ai-skill-permission ${skill.enabled?'enabled':''}" data-ai-skill-toggle="${esc(skill.name)}" aria-pressed="${skill.enabled}"><span class="ai-skill-icon">${icon(skill.python?'terminal':'skill',17)}</span><span><strong>${esc(skill.name)}</strong><small>${esc(skill.description)}</small></span><em>${i18n.t(skill.bundled?'builtInSkill':'importedSkill')}${skill.python?` · ${i18n.t('pythonSkill')}`:''}${skill.fileCount>1?` · ${i18n.t('skillFileCount').replace('{count}',skill.fileCount)}`:''}${skill.toolCount?` · ${i18n.t('skillToolCount').replace('{count}',skill.toolCount)}`:''}</em><i></i></button>`).join('')}</div></div>`;
      const automationContent=automationSettingsMarkup({automations,registry:tools.registry,i18n,formOpen:automationFormOpen,triggerType:automationTriggerType});
      const agentCard=(agent,isNew=false)=>`<details class="ai-agent-card" data-ai-agent-card="${esc(agent.id||'')}" ${isNew?'open':''}><summary><span class="app-icon app-icon-${esc(agent.color||'blue')}">${icon(agent.icon||'agentGeneral',18)}</span><span><strong>${esc(agent.name||i18n.t('newAgent'))}</strong><small>${esc(agent.description||i18n.t('newAgentCopy'))}</small></span><em class="${agent.enabled!==false?'enabled':''}">${i18n.t(agent.enabled!==false?'agentEnabled':'agentDisabled')}</em>${icon('chevron',11)}</summary><div class="ai-agent-editor"><div class="ai-agent-fields"><label><span>${i18n.t('agentName')}</span><input data-agent-name value="${esc(agent.name||'')}"></label><label><span>${i18n.t('agentId')}</span><input data-agent-id value="${esc(agent.id||'')}" ${isNew?'':'disabled'} spellcheck="false"></label><label class="wide"><span>${i18n.t('agentDescription')}</span><input data-agent-description value="${esc(agent.description||'')}"></label><label><span>${i18n.t('agentIcon')}</span><select data-agent-icon>${[['agentGeneral','general'],['agentPlanner','planner'],['agentProductivity','productivity'],['agentWorkspace','workspace'],['agentComputer','computer'],['agentCreator','creator']].map(([value,label])=>`<option value="${value}" ${agent.icon===value?'selected':''}>${i18n.t(`agentIcon_${label}`)}</option>`).join('')}</select></label><label><span>${i18n.t('agentColor')}</span><select data-agent-color>${['blue','purple','green','orange','pink','red','grey','teal'].map(value=>`<option value="${value}" ${agent.color===value?'selected':''}>${value}</option>`).join('')}</select></label><label class="wide"><span>${i18n.t('agentModel')}</span><select data-agent-model><option value="">${i18n.t('inheritMainModel')}</option>${aiAgent.modelOptions().map(model=>`<option value="${esc(model.key)}" ${agent.modelKey===model.key?'selected':''}>${esc(model.providerName)} · ${esc(model.label)}</option>`).join('')}</select></label><label class="wide"><span>${i18n.t('agentInstructions')}</span><textarea data-agent-prompt rows="4">${esc(agent.systemPrompt||'')}</textarea></label></div><section><strong>${i18n.t('agentToolAccess')}</strong><div class="ai-agent-capabilities">${aiAgent.toolApps().map(app=>`<label><input type="checkbox" data-agent-tool="${esc(app.id)}" ${(agent.toolApps||[]).includes(app.id)?'checked':''}><span class="app-icon app-icon-${esc(app.color||'blue')}">${icon(app.icon,12)}</span>${esc(i18n.t(app.title))}</label>`).join('')}</div></section><section><strong>${i18n.t('agentSkillAccess')}</strong><div class="ai-agent-capabilities">${skillRegistry.list().map(skill=>`<label><input type="checkbox" data-agent-skill="${esc(skill.name)}" ${(agent.skills||[]).includes(skill.name)?'checked':''}>${icon('skill',12)}${esc(skill.name)}</label>`).join('')}</div></section><footer><label class="ai-agent-enabled"><input type="checkbox" data-agent-enabled ${agent.enabled!==false?'checked':''}><i></i>${i18n.t('enabled')}</label><span></span>${!agent.builtIn&&!isNew?`<button data-ai-delete-agent>${icon('delete',12)} ${i18n.t('delete')}</button>`:''}<button class="ai-primary" data-ai-save-agent data-new="${isNew}">${icon('check',12)} ${i18n.t('save')}</button></footer></div></details>`;
      const agentContent=`<div class="ai-agent-settings"><header><div><h3>${i18n.t('agentSettings')}</h3><p>${i18n.t('agentSettingsCopy')}</p></div><button data-ai-add-agent>${icon('plus',12)} ${i18n.t('addAgent')}</button></header><div data-ai-agent-list>${agentRegistry.list().map(agent=>agentCard(agent)).join('')}</div></div>`;
      const sectionIcon=settingsSection==='model'?'settings':settingsSection==='usage'?'chart':settingsSection==='tools'?'wrench':settingsSection==='agents'?'user':settingsSection==='automations'?'timer':'skill',sectionCopy=settingsSection==='model'?'providerConfiguration':settingsSection==='usage'?'usageSettingsCopy':settingsSection==='tools'?'toolConfiguration':settingsSection==='agents'?'agentSettingsCopy':settingsSection==='automations'?'automationsPageCopy':'skillConfiguration',sectionContent=settingsSection==='model'?modelContent:settingsSection==='usage'?usageContent:settingsSection==='tools'?toolContent:settingsSection==='agents'?agentContent:settingsSection==='automations'?automationContent:skillContent;
      return `<div class="ai-settings-backdrop" data-ai-close-settings></div><section class="ai-settings-panel ai-settings-root">
        ...
        <div class="ai-settings-layout"><nav><button class="${settingsSection==='model'?'selected':''}" data-ai-settings-section="model">${icon('settings',16)}<span><strong>${i18n.t('modelSettings')}</strong><small>${i18n.t('modelSettingsCopy')}</small></span>${icon('chevron',12)}</button><button class="${settingsSection==='agents'?'selected':''}" data-ai-settings-section="agents">${icon('user',16)}<span><strong>${i18n.t('agentSettings')}</strong><small>${i18n.t('agentSettingsNavCopy')}</small></span>${icon('chevron',12)}</button><button class="${settingsSection==='automations'?'selected':''}" data-ai-settings-section="automations">${icon('timer',16)}<span><strong>${i18n.t('automations')}</strong><small>${i18n.t('automationsPageCopy')}</small></span>${icon('chevron',12)}</button><button class="${settingsSection==='usage'?'selected':''}" data-ai-settings-section="usage">${icon('chart',16)}<span><strong>${i18n.t('usage')}</strong><small>${i18n.t('usageSettingsNavCopy')}</small></span>${icon('chevron',12)}</button><button class="${settingsSection==='tools'?'selected':''}" data-ai-settings-section="tools">${icon('wrench',16)}<span><strong>${i18n.t('toolSettings')}</strong><small>${i18n.t('toolSettingsCopy')}</small></span>${icon('chevron',12)}</button><button class="${settingsSection==='skills'?'selected':''}" data-ai-settings-section="skills">${icon('skill',16)}<span><strong>${i18n.t('skillSettings')}</strong><small>${i18n.t('skillSettingsCopy')}</small></span>${icon('chevron',12)}</button></nav><main>${sectionContent}</main></div>
    ...
    const bindContextControls=()=>{
      ...
      root.querySelectorAll('[data-ai-context-window]').forEach(button=>button.onclick=()=>{const target=semanticContextWindows().find(item=>item.id===button.dataset.aiContextWindow);contextMenuOpen=false;if(target)agentContext.selectWindow(target)});
    ...
    const updateContextUi=()=>{
      ...
      if(workspaceContext){const context=agentContext.snapshot(),windows=semanticContextWindows(),activityApps=activityAppIds.map(id=>tools.registry.get(id)).filter(Boolean);replaceElement(workspaceContext,contextViewMarkup(context,windows,tools,i18n,activityApps))}
      bindContextControls();
    };
    ...
    const syncTaskIndicator=()=>{const button=root.querySelector('[data-ai-workspace-view-menu]'),badge=button?.querySelector('i'),count=systemTasks.snapshot().running||0;if(!button||!badge)return;button.classList.toggle('has-running',count>0);badge.hidden=count===0;badge.textContent=count||''};
    const refreshTaskWorkspace=()=>{const current=root.querySelector('.ai-system-task-workspace');if(!current||!workspaceOpen||workspaceView!=='tasks')return;const scroll=current.querySelector('.ai-system-task-list,.ai-upcoming-automations,.ai-operation-list')?.scrollTop||0,template=document.createElement('template');template.innerHTML=taskWorkspaceMarkup({systemTasks,operationHistory,automations,registry:tools.registry,i18n,view:taskWorkspaceView});const next=template.content.firstElementChild;if(!next)return;current.replaceWith(next);const outer=root.querySelector('[data-ai-app-workspace]');if(outer)outer.dataset.workspaceSignature=`tasks:${i18n.t('dateFormat')}:${taskWorkspaceSignature(systemTasks,operationHistory,{automations,view:taskWorkspaceView})}`;bindSystemTaskControls(next);const list=next.querySelector('.ai-system-task-list,.ai-upcoming-automations,.ai-operation-list');if(list)list.scrollTop=scroll};
    const bindSystemTaskControls=(scope=root)=>{
      scope.querySelectorAll('[data-ai-task-summary-view]').forEach(button=>button.onclick=()=>{taskWorkspaceView=button.dataset.aiTaskSummaryView;refreshTaskWorkspace()});
      scope.querySelectorAll('[data-ai-system-task-open]').forEach(button=>button.onclick=()=>{const task=systemTasks.snapshot().tasks.find(item=>item.id===button.dataset.aiSystemTaskOpen);if(!task?.sessionId||!aiAgent.snapshot().sessions.some(item=>item.id===task.sessionId))return;activeId=task.sessionId;followConversation=true;render({preserveComposer:true,focusComposer:false});focusNotificationTarget({turnId:task.turnId})});
      scope.querySelectorAll('[data-ai-system-task-cancel]').forEach(button=>button.onclick=event=>{event.stopPropagation();systemTasks.cancel(button.dataset.aiSystemTaskCancel)});
      scope.querySelectorAll('[data-ai-system-task-retry]').forEach(button=>button.onclick=event=>{event.stopPropagation();systemTasks.retry(button.dataset.aiSystemTaskRetry).catch(error=>shell.toast(error.message))});
      scope.querySelectorAll('[data-ai-system-task-remove]').forEach(button=>button.onclick=event=>{event.stopPropagation();systemTasks.remove(button.dataset.aiSystemTaskRemove)});
      scope.querySelectorAll('[data-ai-upcoming-expand]').forEach(button=>button.onclick=()=>{const row=button.closest('[data-ai-upcoming-automation]'),details=row?.querySelector(':scope > section'),expanded=button.getAttribute('aria-expanded')!=='true';if(!row||!details)return;button.setAttribute('aria-expanded',String(expanded));button.title=i18n.t(expanded?'hideDetails':'showDetails');row.classList.toggle('expanded',expanded);details.hidden=!expanded});
      scope.querySelectorAll('.ai-upcoming-automation [data-ai-automation-delete]').forEach(button=>button.onclick=async()=>{const rule=automations.get(button.dataset.aiAutomationDelete);if(rule&&await dialog.confirm({title:i18n.t('deleteAutomation'),message:i18n.t('deleteAutomationConfirm').replace('{name}',rule.name),confirmLabel:i18n.t('delete'),danger:true}))automations.remove(rule.id)});
      scope.querySelectorAll('[data-ai-operation-undo]').forEach(button=>button.onclick=async()=>{const record=operationHistory.snapshot().records.find(item=>item.id===button.dataset.aiOperationUndo);if(!record||!await dialog.confirm({title:i18n.t('undoOperation'),message:i18n.t('undoOperationConfirm').replace('{name}',record.label),confirmLabel:i18n.t('undo')}))return;operationHistory.undo(record.id).catch(error=>shell.toast(error.message))});
    };
    const bindAutomationControls=(scope=root)=>{
      scope.querySelector('[data-ai-new-automation]')?.addEventListener('click',()=>{automationFormOpen=true;automationTriggerType='daily';refreshSettingsPanel();queueMicrotask(()=>root.querySelector('[data-ai-automation-name]')?.focus())});
      scope.querySelectorAll('[data-ai-cancel-automation]').forEach(button=>button.onclick=()=>{automationFormOpen=false;refreshSettingsPanel()});
      scope.querySelector('[data-ai-automation-trigger]')?.addEventListener('change',event=>{automationTriggerType=event.target.value;scope.querySelectorAll('[data-ai-automation-field]').forEach(field=>field.hidden=field.dataset.aiAutomationField!==automationTriggerType)});
      scope.querySelector('[data-ai-save-automation]')?.addEventListener('click',()=>{const name=scope.querySelector('[data-ai-automation-name]')?.value.trim(),prompt=scope.querySelector('[data-ai-automation-prompt]')?.value.trim();if(!name||!prompt)return shell.toast(i18n.t('automationRequired'));automations.create({name,trigger:{type:automationTriggerType,time:scope.querySelector('[data-ai-automation-time]')?.value,intervalMinutes:Number(scope.querySelector('[data-ai-automation-interval]')?.value)||60,appId:scope.querySelector('[data-ai-automation-app]')?.value,path:scope.querySelector('[data-ai-automation-path]')?.value.trim(),threshold:Number(scope.querySelector('[data-ai-automation-threshold]')?.value)||80},action:{type:'agent',prompt}});automationFormOpen=false;refreshSettingsPanel()});
      scope.querySelectorAll('[data-ai-automation-toggle]').forEach(input=>input.onchange=()=>automations.setEnabled(input.dataset.aiAutomationToggle,input.checked));
      scope.querySelectorAll('[data-ai-automation-run]').forEach(button=>button.onclick=()=>automations.run(button.dataset.aiAutomationRun,{reason:'manual'}).catch(error=>shell.toast(error.message)));
      scope.querySelectorAll('[data-ai-automation-delete]').forEach(button=>button.onclick=async()=>{const rule=automations.get(button.dataset.aiAutomationDelete);if(rule&&await dialog.confirm({title:i18n.t('deleteAutomation'),message:i18n.t('deleteAutomationConfirm').replace('{name}',rule.name),confirmLabel:i18n.t('delete'),danger:true}))automations.remove(rule.id)});
    };
    const bindSettingsControls=(scope=root)=>{
      ...
      scope.querySelector('[data-ai-add-agent]')?.addEventListener('click',async()=>{const name=await dialog.prompt({title:i18n.t('addAgent'),message:i18n.t('newAgentCopy'),value:''});if(!name)return;try{agentRegistry.create({name,description:'',systemPrompt:'',enabled:true,toolApps:[],skills:[]});refreshSettingsPanel();queueMicrotask(()=>scope.querySelector(`[data-ai-agent-card="${CSS.escape(String(name).toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,''))}"]`)?.setAttribute('open',''))}catch(error){localError=friendlyError(error);render({preserveComposer:true})}});
      scope.querySelectorAll('[data-ai-save-agent]').forEach(button=>button.onclick=()=>{const card=button.closest('[data-ai-agent-card]');if(!card)return;try{const id=card.dataset.aiAgentCard;agentRegistry.update(id,{name:card.querySelector('[data-agent-name]').value,description:card.querySelector('[data-agent-description]').value,icon:card.querySelector('[data-agent-icon]').value,color:card.querySelector('[data-agent-color]').value,modelKey:card.querySelector('[data-agent-model]').value,systemPrompt:card.querySelector('[data-agent-prompt]').value,enabled:card.querySelector('[data-agent-enabled]').checked,toolApps:[...card.querySelectorAll('[data-agent-tool]:checked')].map(item=>item.dataset.agentTool),skills:[...card.querySelectorAll('[data-agent-skill]:checked')].map(item=>item.dataset.agentSkill)});shell.toast(i18n.t('agentSaved'));refreshSettingsPanel()}catch(error){localError=friendlyError(error);render({preserveComposer:true})}});
      scope.querySelectorAll('[data-ai-delete-agent]').forEach(button=>button.onclick=async()=>{const card=button.closest('[data-ai-agent-card]'),agent=agentRegistry.get(card?.dataset.aiAgentCard);if(!agent)return;const approved=await dialog.confirm({title:i18n.t('deleteAgent'),message:i18n.t('deleteAgentConfirm').replace('{name}',agent.name),confirmLabel:i18n.t('delete'),danger:true});if(!approved)return;try{agentRegistry.remove(agent.id);refreshSettingsPanel()}catch(error){localError=friendlyError(error);render({preserveComposer:true})}});
      const skillFolder=scope.querySelector('[data-ai-skill-folder]'),importSkill=scope.querySelector('[data-ai-import-skill]');
      if(importSkill&&skillFolder){importSkill.onclick=()=>skillFolder.click();skillFolder.onchange=async()=>{const files=[...skillFolder.files];if(!files.length)return;importSkill.disabled=true;importSkill.innerHTML=`${icon('refresh',12)} ${i18n.t('importingSkill')}`;try{const entries=await Promise.all(files.map(async file=>({path:file.webkitRelativePath||file.name,type:file.type,bytes:new Uint8Array(await file.arrayBuffer())}))),name=await skillRegistry.installPackage(entries);shell.toast(i18n.t('skillInstalled').replace('{name}',name));localError='';refreshSettingsPanel()}catch(error){localError=friendlyError(error);render({preserveComposer:true})}}}
    ...
    const bind = () => {
      ...
      root.querySelector('[data-ai-switch-compact]')?.addEventListener('click',()=>{
        ...
        agentEntry.open({mode:'compact',source:'view-switch',sessionId:activeId,agentMode:current()?.agentMode||'general',prompt,skillName:selectedSkillName,attachments:composerAttachments});composerAttachments=[];
      ...
      root.querySelectorAll('[data-ai-workspace-view]').forEach(button=>button.onclick=()=>{workspaceOpen=true;workspaceView=button.dataset.aiWorkspaceView;workspaceViewMenuOpen=false;persistWorkspacePrefs();render({preserveComposer:true,preserveConversation:true,focusComposer:false})});
      bindSystemTaskControls();
      ...
      const closeWorkspace=root.querySelector('[data-ai-close-workspace]');if(closeWorkspace)closeWorkspace.onclick=()=>{workspaceViewMenuOpen=false;setWorkspaceOpen(false)};
      ...
      root.querySelector('[data-ai-composer-mode]')?.addEventListener('change',event=>{try{aiAgent.setSessionMode(activeId,event.target.value);localError='';render({preserveComposer:true,focusComposer:true})}catch(error){localError=error.message||String(error);render({preserveComposer:true})}});
    ...
    const offChanged = kernel.bus.on('ai:changed', detail => {
      ...
      if(detail?.sessionId&&detail.sessionId!==activeId){syncSessionIndicators();return}
      if(detail?.sessionId===activeId&&current()?.streaming)return;
      if(settingsOpen){if(settingsSection==='automations'&&automationFormOpen)return;refreshSettingsPanel();return}
      render({preserveComposer:true});
    });
    const offAgent = kernel.bus.on('ai:agent-event', detail => {
      if(detail.sessionId!==activeId){syncSessionIndicators();return}if(settingsOpen)return;
    ...
    const offQuery = kernel.bus.on('agent:query-user',detail=>{if(detail?.sessionId===activeId&&!settingsOpen)render({preserveComposer:true,preserveConversation:true,focusComposer:false});else syncSessionIndicators()});
    const offNotifications = kernel.bus.on('notification:changed',state=>{refreshNotificationPanel(state);syncSessionIndicators()});
    const offContext = kernel.bus.on('agent:context-changed',updateContextUi);
    const offEntry = kernel.bus.on('ai:entry',detail=>{const sessions=aiAgent.snapshot().sessions,requested=String(detail.sessionId||''),utilityEntry=Boolean(detail.workspace||detail.settings);if(requested&&sessions.some(item=>item.id===requested))activeId=requested;else if(!activeId||(!utilityEntry&&current()?.turns?.length))activeId=aiAgent.createSession({agentMode:detail.agentMode});composerDraft=detail.prompt||'';composerAttachments=Array.isArray(detail.attachments)?detail.attachments:[];selectedSkillName=enabledSkills().some(skill=>skill.name===detail.skillName)?detail.skillName:'';skillCommandQuery=null;settingsOpen=Boolean(detail.settings);if(detail.settings)settingsSection=['model','agents','automations','usage','tools','skills'].includes(detail.settingsSection)?detail.settingsSection:'model';if(['workflow','tasks','activity','context'].includes(detail.workspace)){workspaceOpen=true;workspaceView=detail.workspace;persistWorkspacePrefs()}notificationOpen=false;followConversation=true;render({focusComposer:!detail.settings&&!detail.workspace});focusNotificationTarget(detail.target);if(detail.autoSend&&(composerDraft||composerAttachments.length))send()});
    ...
    const offWorkflow = kernel.bus.on('multi-agent:workflow',detail=>{if(detail?.sessionId!==activeId||settingsOpen||workspaceView==='tasks')return;if(detail.workflow?.nodes?.length>1&&detail.workflow.id!==lastOpenedWorkflowId){lastOpenedWorkflowId=detail.workflow.id;workflowFoldState.set(detail.workflow.id,true);workspaceOpen=true;workspaceView='workflow';persistWorkspacePrefs()}render({preserveComposer:true,preserveConversation:true,focusComposer:false})});
    const offSystemTasks=kernel.bus.on('system-tasks:changed',()=>{syncTaskIndicator();if(workspaceOpen&&workspaceView==='tasks'&&!settingsOpen)refreshTaskWorkspace()});
    const offOperationHistory=kernel.bus.on('operation-history:changed',()=>{if(workspaceOpen&&workspaceView==='tasks'&&!settingsOpen)refreshTaskWorkspace()});
    const offAutomations=kernel.bus.on('automations:changed',()=>{if(settingsOpen&&settingsSection==='automations'&&!automationFormOpen)refreshSettingsPanel();if(workspaceOpen&&workspaceView==='tasks'&&!settingsOpen)refreshTaskWorkspace()});
    ...
    const closeWorkspaceViewMenu=event=>{if(!workspaceViewMenuOpen||event.target.closest('[data-ai-workspace-picker]'))return;workspaceViewMenuOpen=false;const picker=root.querySelector('[data-ai-workspace-picker]');picker?.classList.remove('open');picker?.querySelector('[data-ai-workspace-view-menu]')?.setAttribute('aria-expanded','false')};
    root.addEventListener('click',closeContextOnClick);root.addEventListener('keydown',closeContextOnEscape,true);document.addEventListener('pointerdown',closeSkillOnPointerDown,true);document.addEventListener('pointerdown',closeWorkspaceViewMenu,true);
    ensureSession();render();
    return () => { kernel.bus.emit('ai:surface-session',{surface:'full',sessionId:''});unmountActivitySurface();conversationResizeObserver?.disconnect();if(streamingFrame)cancelAnimationFrame(streamingFrame);clearTimeout(workspaceHighlightTimer);root.removeEventListener('click',closeContextOnClick);root.removeEventListener('keydown',closeContextOnEscape,true);document.removeEventListener('pointerdown',closeSkillOnPointerDown,true);document.removeEventListener('pointerdown',closeWorkspaceViewMenu,true);offReady(); offChanged(); offAgent(); offCapability(); offQuery(); offNotifications(); offContext(); offEntry(); offOpenApp();offAppBeforeUpdate();offAppUpdated();offAppBeforeUninstall();offLocale();offSkills();offWorkflow();offSystemTasks();offOperationHistory();offAutomations(); };
  },
};
