...
export default {
  ...
  mount(root, { aiAgent, agentRegistry, multiAgent, i18n, kernel, dialog, shell, clipboard, tools, notifications, agentContext, agentEntry, queryUser, userdata, system, settings, weather, music, browser, browserAutomation, metrics, machine, skillRegistry, systemTasks, automations, operationHistory }) {
    ...
    let taskWorkspaceTab='tasks',taskFilter='all',automationFormOpen=false,automationTriggerType='daily';
    ...
    const render = ({ preserveComposer = false, preserveConversation = false, focusSearch = false, focusComposer = preserveComposer } = {}) => {
      ...
      const workflowSignature=sessionWorkflows.map(workflow=>`${workflow.id}:${workflow.updatedAt}:${workflow.progress}:${expandedWorkflowIds.has(workflow.id)?1:0}:${workflow.nodes.map(node=>`${node.id}:${node.phase}:${node.progress}:${(node.tools||[]).map(tool=>`${tool.id}:${tool.phase}:${tool.updatedAt||tool.finishedAt||0}`).join(',')}`).join(';')}`).join('|')||'empty',tasksSignature=taskWorkspaceSignature(systemTasks,operationHistory,{tab:taskWorkspaceTab,filter:taskFilter}),signature=workspaceView==='workflow'?`workflow:${workflowSignature}`:workspaceView==='tasks'?`tasks:${localeSignature}:${tasksSignature}`:workspaceView==='context'?`context:${localeSignature}:${contextSignature}`:`activity:${localeSignature}:${contextSignature}`,reuseWorkspace=Boolean(previousWorkspace&&workspaceVisible&&previousWorkspace.dataset.workspaceSignature===signature),animateWorkspace=!previousWorkspace;
      const taskMarkup=workspaceView==='tasks'?taskWorkspaceMarkup({systemTasks,operationHistory,registry:tools.registry,i18n,tab:taskWorkspaceTab,filter:taskFilter}):'';
    ...
    const refreshTaskWorkspace=()=>{const current=root.querySelector('.ai-system-task-workspace');if(!current||!workspaceOpen||workspaceView!=='tasks')return;const scroll=current.querySelector('.ai-system-task-list,.ai-operation-list')?.scrollTop||0,template=document.createElement('template');template.innerHTML=taskWorkspaceMarkup({systemTasks,operationHistory,registry:tools.registry,i18n,tab:taskWorkspaceTab,filter:taskFilter});const next=template.content.firstElementChild;if(!next)return;current.replaceWith(next);const outer=root.querySelector('[data-ai-app-workspace]');if(outer)outer.dataset.workspaceSignature=`tasks:${i18n.t('dateFormat')}:${taskWorkspaceSignature(systemTasks,operationHistory,{tab:taskWorkspaceTab,filter:taskFilter})}`;bindSystemTaskControls(next);const list=next.querySelector('.ai-system-task-list,.ai-operation-list');if(list)list.scrollTop=scroll};
    const bindSystemTaskControls=(scope=root)=>{
      scope.querySelectorAll('[data-ai-task-workspace-tab]').forEach(button=>button.onclick=()=>{taskWorkspaceTab=button.dataset.aiTaskWorkspaceTab;refreshTaskWorkspace()});
      scope.querySelectorAll('[data-ai-task-filter]').forEach(button=>button.onclick=()=>{taskFilter=button.dataset.aiTaskFilter;refreshTaskWorkspace()});
    ...
    const offAutomations=kernel.bus.on('automations:changed',()=>{if(settingsOpen&&settingsSection==='automations'&&!automationFormOpen)refreshSettingsPanel()});
