...
import { contextViewMarkup, workspaceMarkup } from './AgentWorkspace.js';
...
const readWorkspacePrefs = () => {
  try {
    ...
    return {
      ...
      view: ['workflow','activity','context'].includes(stored.view) ? stored.view : 'activity',
    };
  } catch {
  ...
  mount(root, { aiAgent, agentRegistry, multiAgent, i18n, kernel, dialog, shell, clipboard, tools, notifications, agentContext, agentEntry, queryUser, userdata, system, settings, weather, music, browser, browserAutomation, metrics, machine, skillRegistry }) {
    ...
    const render = ({ preserveComposer = false, preserveConversation = false, focusSearch = false, focusComposer = preserveComposer } = {}) => {
      ...
      const shellMarkup=`<div class="system-app ai-system-app ${workspaceVisible?'has-app-workspace':''}" style="--agent-workspace-width:${workspaceWidth}px">
        ...
        ${workspaceVisible?workspaceMarkup(tools,i18n,{animate:animateWorkspace,signature,view:workspaceView,viewMenuOpen:workspaceViewMenuOpen,context:workspaceContext,windows:contextWindows,activityApps,activityAppId,workflows:sessionWorkflows,expandedWorkflowIds}):''}
      </div>`;
    ...
    const settingsMarkup = () => {
      ...
      const sectionIcon=settingsSection==='model'?'settings':settingsSection==='usage'?'chart':settingsSection==='tools'?'wrench':settingsSection==='agents'?'user':'skill',sectionCopy=settingsSection==='model'?'providerConfiguration':settingsSection==='usage'?'usageSettingsCopy':settingsSection==='tools'?'toolConfiguration':settingsSection==='agents'?'agentSettingsCopy':'skillConfiguration',sectionContent=settingsSection==='model'?modelContent:settingsSection==='usage'?usageContent:settingsSection==='tools'?toolContent:settingsSection==='agents'?agentContent:skillContent;
