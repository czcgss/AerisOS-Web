...
export default {
  ...
  mount(root, { aiAgent, i18n, kernel, dialog, shell, clipboard, tools, notifications, agentContext, agentEntry, agentTasks }) {
    ...
    const contextMarkup=()=>{
      ...
      if(!context&&!windows.length)return'';
      ...
      return`<div class="ai-native-context-wrap"><div class="ai-native-context"><button class="ai-context-current" data-ai-context-selector aria-expanded="${contextMenuOpen}"><span class="app-icon app-icon-${app?.color||'blue'}">${icon(app?.icon||(desktop?'desktop':'sparkles'),14)}</span><span><small>${i18n.t('systemContext')}</small><strong>${esc(label)}</strong></span>${context?.selection?.text?`<em>${i18n.t('selectedText')}</em>`:''}${icon('chevron',10)}</button>${context?`<button data-ai-clear-context aria-label="${i18n.t('clearContext')}">${icon('close',10)}</button>`:''}</div>${picker}</div>`;
    };
