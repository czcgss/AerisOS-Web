...
export default {
  ...
  mount(root, { aiAgent, i18n, kernel, dialog, shell, clipboard, tools, notifications, agentContext, agentEntry, userdata, system, settings, weather, music, metrics, machine, skillRegistry }) {
    ...
    const toolCard = ({ name, toolCallId, args = {}, details = null, output = '', isError = false, turnStatus = 'running' }) => {
      ...
      const app=tools.registry.get(metadata.appId||execution.appId),label=execution.label||metadata.label||skillTool?.label||name,skillName=execution.skillId||skillTool?.skillName||args?.name||'';
      return `<section class="ai-tool-call ${skillTool?'ai-skill-tool-call':''} ${skillTool?.kind==='loader'?'ai-skill-loader-call':''} ai-tool-${phase}" data-tool-call="${esc(toolCallId)}"><header>${toolIcon(metadata.appId||execution.appId,skillTool)}<span><strong>${esc(label)}</strong><small>${skillTool?`${i18n.t(skillTool.kind==='loader'?'skillLoader':'skillTool')}${skillName?` · ${esc(skillName)}`:''}`:app?i18n.t(app.title):i18n.t('systemTool')}</small></span>${metadata.risk==='high'||execution.risk==='high'?`<b class="ai-risk-badge">${icon('lock',10)} ${i18n.t('approvalRequired')}</b>`:''}<em>${i18n.t(`toolStatus_${phase}`)}</em></header>${Object.keys(displayArgs).length?`<details><summary>${i18n.t('toolParameters')}</summary><pre data-copyable>${esc(JSON.stringify(displayArgs,null,2))}</pre></details>`:''}${output?`<details class="ai-tool-output"><summary>${i18n.t('toolResult')}</summary><p data-copyable>${esc(output).slice(0,1200)}</p></details>`:''}</section>`;
    };
    ...
    const turnMarkup = (turn, index, lastTurnIndex, session) => {
      ...
      const assistantActions=actionsAvailable?`<footer class="ai-message-actions"><button data-ai-copy-turn="${esc(turn.id)}" data-ai-copy-role="assistant" title="${i18n.t('copyMessage')}">${icon('copy',14)}</button></footer>`:'';
    ...
    const settingsMarkup = () => {
      ...
      const modelContent=`<div class="ai-settings-scroll"><label><span>${i18n.t('apiBaseUrl')}</span><input data-ai-base type="url" value="${esc(config.baseUrl)}" spellcheck="false" autocomplete="off"></label><label><span>${i18n.t('apiKey')}</span><div class="ai-secret-field"><input data-ai-key type="text" value="${esc(config.apiKey)}" spellcheck="false" autocomplete="off" data-lpignore="true"><button data-ai-reveal type="button">${icon('eye',15)}</button></div><small>${i18n.t('apiKeyStoredCopy')}</small></label><label><span>${i18n.t('aiModel')}</span><input data-ai-model value="${esc(config.model)}" spellcheck="false" autocomplete="off"></label><fieldset class="ai-reasoning-setting"><legend>${i18n.t('reasoningEffort')}</legend><small>${i18n.t('reasoningEffortCopy')}</small><div>${['low','medium','high'].map(level=>`<label><input type="radio" name="ai-reasoning-effort" value="${level}" ${config.reasoningEffort===level?'checked':''}><span>${i18n.t(`reasoning_${level}`)}</span></label>`).join('')}</div></fieldset><label><span>${i18n.t('systemInstructions')}</span><textarea data-ai-prompt rows="5">${esc(config.systemPrompt)}</textarea></label><div class="ai-storage-card">${icon('folder',18)}<span><strong>${i18n.t('conversationStorage')}</strong><code>localStorage · ${AI_STATE_STORAGE_KEY}</code></span></div></div>`;
      ...
      const sectionIcon=settingsSection==='model'?'settings':settingsSection==='tools'?'wrench':'skill',sectionCopy=settingsSection==='model'?'providerConfiguration':settingsSection==='tools'?'toolConfiguration':'skillConfiguration';
      return `<div class="ai-settings-backdrop" data-ai-close-settings></div><section class="ai-settings-panel ai-settings-root">
        ...
        <div class="ai-settings-layout"><nav><button class="${settingsSection==='model'?'selected':''}" data-ai-settings-section="model">${icon('settings',16)}<span><strong>${i18n.t('modelSettings')}</strong><small>${i18n.t('modelSettingsCopy')}</small></span>${icon('chevron',12)}</button><button class="${settingsSection==='tools'?'selected':''}" data-ai-settings-section="tools">${icon('wrench',16)}<span><strong>${i18n.t('toolSettings')}</strong><small>${i18n.t('toolSettingsCopy')}</small></span>${icon('chevron',12)}</button><button class="${settingsSection==='skills'?'selected':''}" data-ai-settings-section="skills">${icon('skill',16)}<span><strong>${i18n.t('skillSettings')}</strong><small>${i18n.t('skillSettingsCopy')}</small></span>${icon('chevron',12)}</button></nav><main>${settingsSection==='model'?modelContent:settingsSection==='tools'?toolContent:skillContent}</main></div>
    ...
    const bind = () => {
      ...
      root.querySelector('[data-ai-import-skill]')?.addEventListener('click',async()=>{const raw=await dialog.prompt({title:i18n.t('importSkill'),message:i18n.t('importSkillCopy'),placeholder:'---\nname: my-skill\ndescription: …\n---\n\n# Instructions',multiline:true,submitLabel:i18n.t('import')});if(!raw)return;try{const name=skillRegistry.install(raw);shell.toast(i18n.t('skillInstalled').replace('{name}',name));render({preserveComposer:true})}catch(error){localError=friendlyError(error);render({preserveComposer:true})}});
