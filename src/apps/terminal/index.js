...
export default{
  ...
  mount(root,{kernel,system,i18n,clipboard,shell,agentEntry}){
    ...
    function createSession(){
      const owner=Symbol('terminal-tab'),port=acquireTerminalPort(system,owner);
      ...
      pane.oncontextmenu=event=>{event.preventDefault();event.stopPropagation();show(id);const bounds=root.getBoundingClientRect();menu.style.left=`${Math.max(8,Math.min(event.clientX-bounds.left,bounds.width-150))}px`;menu.style.top=`${Math.max(48,Math.min(event.clientY-bounds.top,bounds.height-126))}px`;menu.querySelector('[data-terminal-copy]').disabled=!terminal.hasSelection();menu.querySelector('[data-terminal-paste]').disabled=!clipboard.value;menu.hidden=false};
