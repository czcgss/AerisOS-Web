# Internal Aeris Widget Runtime reference
...
```js
{
  ...
  files: {
    ...
    "widget.js": "AerisWidget.ready.then(() => {});",
    "locales/en.json": "{}",
    "locales/zh.json": "{}"
  }
}
...
Wait for `AerisWidget.ready`, then use only declared capabilities:

- `AerisWidget.state.get()`, `set(value)`, `patch(value)`, and `subscribe(listener)` require `storage`.
...
```js
AerisWidget.ready.then(() => {
  AerisWidget.errors.subscribe(error => renderError(error.message));
  AerisWidget.data.subscribe('music', snapshot => renderMusic(snapshot));
});
```
