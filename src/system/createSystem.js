...
export async function createSystem(root) {
  ...
  const skillRegistry=kernel.register('skillRegistry',new SkillRegistryService({storage:localStorage,bundledSkills:[createAppSkill(appStudio,queryUser),createWidgetSkill(widgetStudio),createThemeSkill(themeStudio)]}));
  ...
  const context={kernel,settings,themeRuntime,themeStudio,i18n,dialog,machine,system,metrics,tools,aiAgent,agentContext,agentEntry,queryUser,userdata,clipboard,weather,music,notifications,registry,appRuntime,appStudio,skillRegistry,widgetRegistry,widgetRuntime,widgetStudio};
