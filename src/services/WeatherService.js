...
export class WeatherService{
  ...
  async lookup(query,signal){
    const match=(await this.search(query,{signal}))[0];
    ...
    return{location,data:await this.#forecast(location,signal)};
  }
  async refresh(force=false){
    ...
    try{
      this.data=await this.#forecast(this.location);this.storage.setItem(CACHE_KEY,JSON.stringify(this.data));return this.data;
