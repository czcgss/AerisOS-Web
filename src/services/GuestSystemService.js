...
export class GuestSystemService {
  ...
  async runAgentCommand(command, timeout = 60000, signal = null) {
    this.requireReady();
  ...
  async mkdir(path, { priority = true } = {}) { let result;try{result=await this.execChecked(`mkdir -p ${quote(path)}`,12000,{priority})}catch(error){if(!/timed out/i.test(error.message))throw error;const verified=priority?await this.execInteractive(`[ -d ${quote(path)} ]`,6000):await this.exec(`[ -d ${quote(path)} ]`,6000);if(verified.code!==0)throw error;result={output:'',code:0}}this.#changed(path);return result; }
