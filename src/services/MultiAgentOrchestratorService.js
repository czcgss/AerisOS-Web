...
export class MultiAgentOrchestratorService{
  constructor({registry,storage=globalThis.localStorage}={}){this.registry=registry;this.storage=storage;this.workflows=[];this.runner=null;this.controllers=new Map()}
  ...
  directory(){return this.registry.list({enabledOnly:true}).map(({id,name,description,toolApps,skills})=>({id,name,description,toolApps,skills}))}
  ...
  prompt(){const directory=this.directory();if(!directory.length)return'';return`You are the Main Agent and orchestrator of an Aeris multi-agent system. You own the user conversation, planning, delegation, and final synthesis, but you never execute operating-system or application actions yourself. Registered isolated worker Agents:\n${directory.map(item=>`- ${item.id} (${item.name}): ${item.description}`).join('\n')}\nDelegate every request that inspects or changes Aeris apps, files, settings, extensions, media, or Linux state with aeris_delegate, including simple single-action requests. For questions requiring no system action, answer directly. Put independent assignments in one tasks array so they run concurrently. Send each worker only the minimum explicit context it needs; workers cannot see this conversation. Synthesize worker results into one answer and never expose internal protocol noise.`}
