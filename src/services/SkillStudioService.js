...
export class SkillStudioService {
  ...
  tool(){
    ...
    return{name:'aeris_skill_studio',label:'Aeris Skill Studio',description:'Create, inspect, validate, install, update, list, and enable or disable Aeris Skill packages. Available only after loading the skill-creator Skill.',parameters:Type.Object({
      ...
      draftId:optional('Validated draft id returned by validate.'),name:optional('Exact lowercase hyphenated Skill name.'),path:optional('For inspect, an exact package-relative file path. Omit to inspect the complete text package.'),packageJson:optional('For validate, the complete Aeris Skill source package as JSON.'),enabled:Type.Optional(Type.Boolean({description:'For set_enabled, whether the installed Skill is available to Agent.'})),
      ...
      if(input.type==='install'||input.type==='update'){
        ...
        const outcome=await this.approvalService.runProtected({toolCallId,name:'aeris_skill_studio',label:`Update ${name}`,appId:'ai',operation:'update_skill',params:{name,files:Object.keys(draft.source.files)},approvalMessage:`Update the installed Skill “${name}”?\n\nIts validated instructions, references, assets, and scripts will be replaced. Its enabled state will be preserved.`},signal,onUpdate,()=>this.skillRegistry.installPackage(draft.entries));
  ...
  #parsePackage(raw){
    ...
    for(const [rawPath,rawContent] of Object.entries(files)){
      ...
      if(path.startsWith('scripts/')&&!/\.py$/i.test(path))throw new Error(`Aeris Skill scripts must be Python files: ${path}`);
      ...
      if(/\.py$/i.test(path)){if(!/(?:async\s+def|def)\s+main\s*\(/.test(content))throw new Error(`Python Skill script must define main(input): ${path}`);if(FORBIDDEN_IMPORT.test(content))throw new Error(`Python Skill script imports a capability Aeris does not expose: ${path}`)}
    }
