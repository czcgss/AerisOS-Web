...
export class AutomationService{
  ...
  async run(id,{reason='manual'}={}){const rule=this.rules.find(item=>item.id===id);if(!rule)throw new Error('Automation not found.');if(!this.runner)throw new Error('The Agent runtime is unavailable.');if(rule.runningTaskId){const active=this.tasks?.snapshot().tasks.find(task=>task.id===rule.runningTaskId);if(active&&!terminal.has(active.status))return active}
    const sessionId=this.runner.createSession({title:rule.name,origin:'automation',automation:{id:rule.id,name:rule.name,triggerType:rule.trigger.type,triggerReason:reason}});const task=this.tasks?.begin({kind:'automation',origin:'automation',automationId:rule.id,sessionId,title:rule.name,taskContent:rule.action.prompt,prompt:rule.action.prompt,triggerReason:reason,progress:3});rule.runningTaskId=task?.id||'';rule.lastRunAt=Date.now();rule.lastStatus='running';rule.updatedAt=Date.now();this.#save();
    try{await this.runner.send(sessionId,rule.action.prompt,{source:'automation'});rule.lastStatus='completed';rule.lastError='';return task}catch(error){rule.lastStatus='failed';rule.lastError=error.message||String(error);if(task)this.tasks?.update(task.id,{status:'failed',error:rule.lastError});throw error}finally{rule.runningTaskId='';rule.updatedAt=Date.now();this.#save()}}
