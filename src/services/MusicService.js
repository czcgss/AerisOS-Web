...
const LIBRARY='/mnt/future/Music';
...
export class MusicService{
  ...
  start(){
    ...
    this.kernel.bus.on('guest:ready',()=>{clearTimeout(this.readyRefreshTimer);this.readyRefreshTimer=setTimeout(()=>this.refresh(),2400)});this.kernel.bus.on('filesystem:changed',detail=>{if(detail?.path===LIBRARY||detail?.path?.startsWith(`${LIBRARY}/`))this.refresh()});
    if(this.system.ready)this.refresh();
  }
  stop(){clearTimeout(this.readyRefreshTimer);this.audio.pause();this.#release()}
  clearData(){this.stop();this.audio.removeAttribute('src');this.audio.load();this.tracks=[];this.categories=[];this.current=null;this.error='';this.volume=.72;this.audio.volume=this.volume;localStorage.removeItem('future.music.volume');this.#emit()}
  ...
  setVolume(value){this.volume=Math.max(0,Math.min(1,Number(value)||0));this.audio.volume=this.volume;localStorage.setItem('future.music.volume',String(this.volume));this.#emit('music:volume')}
