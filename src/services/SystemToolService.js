...
export class SystemToolService {
    ...
    return this.apps().map(app => {
      ...
      execute: async (toolCallId, input, signal, onUpdate) => {
        ...
        try {
          ...
          if(definition.mutates){const task=this.systemTasks?.snapshot().tasks.find(item=>item.sessionId===state.sessionId&&(!state.turnId||item.turnId===state.turnId));this.operationHistory?.record({definitionName:definition.name,appId:definition.appId,operation:definition.operation,label:definition.label,params:auditValue(params),result:definition.undo?result:auditValue(result),sessionId:state.sessionId,turnId:state.turnId,agentId:state.agentId,agentName:state.agentName,source:task?.origin==='automation'?'automation':'agent',undoable:Boolean(definition.undo)})}
  ...
  #registerBuiltins() {
    ...
    this.#register({ name:'notes_create',appId:'notes',operation:'create',mutates:true,label:'Create note',description:'Create a local note in Aeris Notes.',parameters:object({title:Type.String(),content:Type.String(),pinned:Type.Optional(Type.Boolean())}),execute:async params=>{const notes=await this.userdata.load('notes',[]),note={id:crypto.randomUUID(),title:safeName(params.title),content:String(params.content),pinned:!!params.pinned,updatedAt:Date.now()};notes.unshift(note);await this.userdata.save('notes',notes);return note},undo:async note=>{const notes=await this.userdata.load('notes',[]);await this.userdata.save('notes',notes.filter(item=>item.id!==note.id));return note},success:note=>`Created note “${note.title}”.` });
    ...
    this.#register({ name:'files_write',appId:'files',operation:'write_file',risk:'high',mutates:true,label:'Write file',description:'Create or replace a UTF-8 text file in the Aeris filesystem. Requires approval because an existing file may be overwritten.',parameters:object({path:Type.String(),content:Type.String()}),approval:params=>`Create or replace “${params.path}”?`,execute:async({path,content})=>{path=mutableAerisPath(path);await this.system.writeChunked(path,content,{priority:true});return{path,bytes:new TextEncoder().encode(content).length}},success:result=>`Saved ${result.path} (${result.bytes} bytes).` });
    this.#register({ name:'files_create_folder',appId:'files',operation:'create_folder',mutates:true,label:'Create folder',description:'Create a folder inside /home/aeris or /mnt/aeris.',parameters:object({path:Type.String()}),execute:async({path})=>{path=mutableAerisPath(path);await this.system.mkdir(path,{priority:true});return{path}},undo:async result=>this.system.trash(result.path,{priority:true}),success:result=>`Created folder ${result.path}.` });
    this.#register({ name:'files_rename',appId:'files',operation:'rename',mutates:true,label:'Rename item',description:'Rename a file or folder without moving it to another directory.',parameters:object({path:Type.String(),name:Type.String()}),execute:async({path,name})=>{path=mutableAerisPath(path);const target=await this.system.rename(path,aerisName(name),{priority:true});return{path:target,previousPath:path}},undo:async result=>this.system.rename(result.path,result.previousPath.split('/').at(-1),{priority:true}),success:result=>`Renamed the item to ${result.path}.` });
    this.#register({ name:'files_copy',appId:'files',operation:'copy',mutates:true,label:'Duplicate item',description:'Duplicate a file or folder beside the original and choose a non-conflicting name.',parameters:object({path:Type.String()}),execute:async({path})=>({path:await this.system.copy(mutableAerisPath(path),{priority:true})}),undo:async result=>this.system.trash(result.path,{priority:true}),success:result=>`Created ${result.path}.` });
    ...
    this.#register({name:'browser_type',appId:'browser',operation:'type',risk:'high',mutates:true,label:'Type into browser',description:'Type text into an indexed Browser Use field. Requires approval because text is transmitted to the displayed website.',parameters:object({index:Type.Number({minimum:0}),text:Type.String(),reason:optionalString('Why this text must be entered')}),approval:params=>`Allow Aeris AI to type into browser element ${params.index}?\n\n${params.text}`,execute:async({index,text},signal)=>this.browserAutomation.execute('type',{index,text},signal),success:()=>`Entered text in the browser field.`});
