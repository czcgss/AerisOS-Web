...
export class SystemToolService {
  ...
  #registerBuiltins() {
    ...
    this.#register({ name:'calendar_create_event',appId:'calendar',operation:'create_event',label:'Create calendar event',description:'Create an event in the Aeris Calendar. Use local ISO date-time values such as 2026-08-05T20:00.',parameters:object({title:Type.String(),start:Type.String(),end:optionalString('End local date-time; defaults to one hour after start'),calendarId:optionalString('personal or work'),location:optionalString('Location'),notes:optionalString('Notes'),allDay:Type.Optional(Type.Boolean())}),execute:async params=>{const state=migrateCalendar(await this.userdata.load('calendar',null)),start=parseLocal(params.start),end=params.end?parseLocal(params.end):new Date(start.getTime()+3600000),event={id:crypto.randomUUID(),title:safeName(params.title),calendarId:params.calendarId||'personal',start:localDateTime(start),end:localDateTime(end),allDay:!!params.allDay,location:safeName(params.location),notes:String(params.notes||''),url:'',attendees:'',repeat:'none',alert:'15m'};state.events.push(event);await this.userdata.save('calendar',state);return event},success:event=>`Created “${event.title}” for ${event.start}.` });
    ...
    this.#register({ name:'reminders_create',appId:'reminders',operation:'create',label:'Create reminder',description:'Create an Aeris reminder.',parameters:object({title:Type.String(),due:optionalString('Optional YYYY-MM-DD due date'),priority:Type.Optional(Type.Boolean())}),execute:async params=>{const items=await this.userdata.load('reminders',[]),item={id:crypto.randomUUID(),title:safeName(params.title),done:false,due:params.due||'',priority:!!params.priority,createdAt:Date.now()};items.push(item);await this.userdata.save('reminders',items);return item},success:item=>`Created reminder “${item.title}”.` });
