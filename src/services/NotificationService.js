    ...
    try {
      ...
      for (const event of occurrences(calendar.events, calendar.calendars, rangeStart, rangeEnd)) {
        ...
        if (alertAt <= now.getTime() && alertAt >= now.getTime() - LATE_WINDOW && !this.state.delivered[sourceKey]) {
          ...
          pending.push({ appId: 'calendar', sourceKey, title: event.title || this.i18n.t('calendarEvent'), message: event.location ? `${start} · ${event.location}` : start, occurredAt: alertAt, context:{appId:'calendar',label:event.title,resource:{kind:'event',id:event.id,uri:`future://calendar/events/${event.id}`,name:event.title,date:event.start,metadata:{start:event.start,end:event.end,location:event.location,calendarId:event.calendarId}}} });
        }
      }
      ...
      for (const reminder of reminders) {
        ...
        if (Number.isFinite(dueAt) && dueAt <= now.getTime() && dueAt >= now.getTime() - LATE_WINDOW && !this.state.delivered[sourceKey]) {
          pending.push({ appId: 'reminders', sourceKey, title: reminder.title || this.i18n.t('reminder'), message: this.#formatDateTime(new Date(dueAt), false), occurredAt: dueAt, context:{appId:'reminders',label:reminder.title,resource:{kind:'reminder',id:reminder.id,uri:`future://reminders/items/${reminder.id}`,name:reminder.title,date:reminder.due,metadata:{dueTime:reminder.dueTime||'09:00',priority:!!reminder.priority}}} });
        }
      }
