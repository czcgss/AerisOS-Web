...
export class AppStudioService {
  ...
  tool(){
    ...
    return {
      name:'future_app_studio',
      label:'Future App Studio',
      description:'Inspect source, validate, install, update, list, and uninstall sandboxed Future app packages without using the terminal. Updating and uninstalling require explicit user approval. This tool is available only after loading the create-app skill.',
      parameters:Type.Object({
        ...
        icon:optional('Future icon id.'),color:optional('Future color: aqua, blue, green, grey, orange, pink, purple, red, or yellow.'),
        ...
        mainHtml:optional('Main view HTML fragment.'),mainCss:optional('Main view CSS.'),mainJavaScript:optional('Main view JavaScript using the Future SDK.'),
        activityHtml:optional('Agent Activity view HTML fragment.'),activityCss:optional('Agent Activity view CSS.'),activityJavaScript:optional('Agent Activity JavaScript using the Future SDK.'),
      ...
      execute:async(toolCallId,input,signal,onUpdate)=>{
        ...
        if(input.type==='install'){
          ...
          return textResult(`Installed “${manifest.name.en}” (${manifest.id}) in Future. Its Main and Agent Activity views are now available.`,{skillId:'create-app',operation:'install',phase:'completed',result:{appId:manifest.id,manifest}});
        }
        if(input.type==='update'){
          ...
          const title=record.manifest.name.en||appId,outcome=await this.approvalService.runProtected({toolCallId,name:'future_app_studio',label:`Update ${title}`,appId,operation:'update',params:{id:appId,version:draft.package.manifest.version},approvalMessage:`Update “${title}” (${appId}) to version ${draft.package.manifest.version}?\n\nThe validated application package will be replaced. Existing app data will be preserved.`},signal,onUpdate,()=>this.appRuntime.install(draft.package,{replace:true,preserveState:true}));
        ...
        if(input.type==='uninstall'){
          ...
          if(!this.approvalService)throw new Error('Future approval service is unavailable.');
          const title=record.manifest.name.en||appId,outcome=await this.approvalService.runProtected({toolCallId,name:'future_app_studio',label:`Uninstall ${title}`,appId,operation:'uninstall',params:{id:appId},approvalMessage:`Uninstall “${title}” (${appId})?\n\nIts application package and saved state will be permanently removed.`},signal,onUpdate,()=>this.appRuntime.uninstall(appId));
          if(!outcome.approved)return textResult('The user denied the app uninstall request.',{...outcome.state,skillId:'create-app'});
          return textResult(`Uninstalled “${title}” (${appId}) from Future.`,{...outcome.state,skillId:'create-app',result:{appId,uninstalled:true}});
        }
  ...
  #assertDesignLanguage(input){
    for(const [field,scriptField,label] of [['mainCss','mainJavaScript','Main'],['activityCss','activityJavaScript','Activity']]){
      ...
      if(missing.length)throw new Error(`${label} CSS must use Future design tokens: ${missing.join(', ')}.`);
      ...
      if(!/\bFuture\s*\.\s*environment\s*\.\s*subscribe\s*\(/.test(String(input[scriptField]||'')))throw new Error(`${label} must subscribe to Future.environment so theme and locale changes apply immediately.`);
    }
  }
